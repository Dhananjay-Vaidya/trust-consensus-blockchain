...
OUTPUT_DIR = "new_res"
SCENARIOS = [
    ("scenarios/smart_grid.yaml",      "Smart Grid",      "cra"),
    ("scenarios/healthcare_iot.yaml",  "Healthcare IoT",  "tdp"),
    ("scenarios/industrial_iot.yaml",  "Industrial IoT",  "bfi"),
]
...
def run_scenario(yaml_path: str, attack_override: str) -> Optional[str]:
    ...
    pattern = f"results/{nodes}_{episodes}_marl_{attack_override}_{pct}_*_episode_metrics.csv"
    matches = sorted(glob.glob(pattern))
    if matches:
        return matches[-1]
    # Fallback
    pattern2 = f"results/{nodes}_{episodes}_marl_{attack_override}_{pct}_episode_metrics.csv"
