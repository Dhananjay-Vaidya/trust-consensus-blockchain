...
Usage:
    python -m analysis.run_malicious_sweep
"""
...
AGENTS = ["rl", "drl", "marl"]
OUTPUT_DIR = "new_res"
RESULTS_CSV = "results/malicious_sweep_results.csv"
...
def run_single(agent: str, malicious: float) -> Optional[float]:
    ...
    pattern = f"results/16_100_{agent}_cra_{pct}_*_episode_metrics.csv"
    matches = sorted(glob.glob(pattern))
    if not matches:
        pattern2 = f"results/16_100_{agent}_cra_{pct}_episode_metrics.csv"
