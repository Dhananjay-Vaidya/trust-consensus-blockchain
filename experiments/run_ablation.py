...
def _result_csv(nodes: int, episodes: int, agent: str, attack: str,
    ...
    return Path("experiments/results") / f"{nodes}_{episodes}_{agent}_{attack}_{malicious}_seed{seed}_episode_metrics.csv"
...
def run_variant(variant: str, extra_args: List[str], seed: int) -> Optional[float]:
    ...
    if not csv_path.exists():
        ...
        csv_path = Path("experiments/results") / "16_100_marl_cra_30_episode_metrics.csv"
    if csv_path.exists():
