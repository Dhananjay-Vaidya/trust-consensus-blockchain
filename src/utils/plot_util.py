...
def plot_cumulative_reward(episodes, rewards, agent_type, attack):
    ...
    plt.grid(True)
    plt.tight_layout()
    plt.savefig(_output_path(f'cumulative_reward_vs_episodes_{agent_type}_{attack}.png'))
    plt.close()
...
def plot_f1_score(episodes, f1_scores, agent_type, attack):
    ...
    plt.grid(True)
    plt.tight_layout()
    plt.savefig(_output_path(f'f1_score_vs_episodes_{agent_type}_{attack}.png'))
    plt.close()
...
def plot_blockchain_length(episodes, blockchain_lengths, agent_type, attack):
    ...
    plt.grid(True)
    plt.tight_layout()
    plt.savefig(_output_path(f'blockchain_length_vs_episodes_{agent_type}_{attack}.png'))
    plt.close()
...
def plot_throughput(episodes, throughputs, agent_type, attack):
    ...
    plt.grid(True)
    plt.tight_layout()
    plt.savefig(_output_path(f'throughput_vs_episodes_{agent_type}_{attack}.png'))
    plt.close()
...
def plot_confusion_matrix_from_labels(y_true, y_pred, agent_type, attack_mode, title="Confusion Matrix"):
    ...
    plt.tight_layout()
    plt.savefig(_output_path(f"confusion_matrix_{agent_type}_{attack_mode}.png"))
    plt.close()
