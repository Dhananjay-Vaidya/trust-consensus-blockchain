...
from src.abac.abac import ABAC
from src.abac.fhe import FullyHomomorphicEncryption
from src.trust.trust import TrustManager
from src.rewards.reward import RewardSystem
from src.consensus.tdcb import TDCB
from src.agents.drl_d3p import DDQN
from src.agents.marl_agent import MARLAgent
from src.utils.plot_util import (plot_cumulative_reward, plot_f1_score,
                       plot_blockchain_length, plot_throughput,
                       plot_confusion_matrix_from_labels)
from src.attacks.attack_util import apply_attack, reset_attack_instances
...
def _build_consensus(consensus_type: str) -> Any:
    ...
    if consensus_type == "pbft":
        from src.baselines.pbft_baseline import PBFTConsensus
        return PBFTConsensus()
    elif consensus_type == "static_dpos":
        from src.baselines.static_dpos_baseline import StaticDPoSConsensus
        return StaticDPoSConsensus()
    elif consensus_type == "majority":
        from src.baselines.majority_vote_baseline import MajorityVoteConsensus
        return MajorityVoteConsensus()
    elif consensus_type == "random":
        from src.baselines.random_delegation_baseline import RandomDelegation
...
class SimulationManager:
    ...
    def _emit_event(self, event: Dict[str, Any]) -> None:
        if self._stream_file is not None:
            self._stream_file.write(json.dumps(event) + "\n")
    ...
    def run_simulation(self) -> None:
        ...
        os.makedirs(RESULTS_DIR, exist_ok=True)
        self._stream_file = open(f"{RESULTS_DIR}/{self._run_id}_stream.jsonl", "w", encoding="utf-8")
        ...
        pd.DataFrame(data).to_csv(f"{RESULTS_DIR}/{prefix}_episode_metrics.csv", index=False)
        ...
        pd.DataFrame(param_data).to_csv(f"{RESULTS_DIR}/{prefix}_simulation_parameters.csv", index=False)
